...
void SourceMap::updateDirectoryStructure() 
{
    // Clear the existing directory structure
    m_directoryStructure.clear();
    
    // Add the working directory as the root
    DirectoryEntry rootEntry;
    rootEntry.name = std::filesystem::path(m_workingDir).filename().string();
    rootEntry.path = m_workingDir;
    rootEntry.isDirectory = true;
    m_directoryStructure[m_workingDir] = rootEntry;
    
    // Process all files in the cache
    for (const auto& [path, fileTags] : m_fileCache) {
        std::filesystem::path filePath(path);
        std::filesystem::path dirPath = filePath.parent_path();
        
        // Ensure all parent directories exist in the structure
        std::filesystem::path currentPath = m_workingDir;
        std::vector<std::filesystem::path> dirs;
        
        // Build a path from working dir to the parent of this file
        std::filesystem::path relativePath;
        try {
            relativePath = std::filesystem::relative(dirPath, m_workingDir);
        ...
        if (relativePath.empty() || relativePath == ".") {
            // File is directly in working directory, add to root
            m_directoryStructure[m_workingDir].children.push_back(path);
            continue;
        }
        ...
        // Add all the directories to the structure
        std::string parentPath = m_workingDir;
        for (const auto& dir : dirs) {
            std::string dirPathStr = dir.string();
            
            // Add directory if it doesn't exist
            if (m_directoryStructure.find(dirPathStr) == m_directoryStructure.end()) {
                DirectoryEntry dirEntry;
                dirEntry.name = dir.filename().string();
                dirEntry.path = dirPathStr;
                dirEntry.isDirectory = true;
                m_directoryStructure[dirPathStr] = dirEntry;
                
                // Add as child to parent
                m_directoryStructure[parentPath].children.push_back(dirPathStr);
            }
        ...
        m_directoryStructure[dirPath.string()].children.push_back(path);
    }
}
